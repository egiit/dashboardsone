...
FROM weekly_prod_sch_detail a
LEFT JOIN order_po_listing b  ON b.MO_NO = :moNo
LEFT JOIN item_siteline d ON a.SCHD_ID_SITELINE = d.ID_SITELINE
LEFT JOIN weekly_prod_schedule e ON a.SCH_ID = e.SCH_ID
...
export const QueryQcEndlineDaily = `SELECT *,ROUND(n.PLAN_MP*n.PLAN_WH*60/n.PLAN_SEW_SMV) PLAN_TARGET,
ROUND(n.PLAN_MP_OT*n.PLAN_WH_OT*60/n.PLAN_SEW_SMV) PLAN_TARGET_OT,
ROUND(n.ACT_MP*n.PLAN_WH*60/n.PLAN_SEW_SMV) ACT_TARGET,
ROUND(n.ACT_MP_OT*n.PLAN_WH_OT*60/n.PLAN_SEW_SMV) ACT_TARGET_OT
FROM (
SELECT a.SCHD_ID, a.SCH_ID, a.SCHD_PROD_DATE, e.ID_SITELINE,  d.SITE_NAME, d.LINE_NAME, e.SHIFT,
b.ORDER_REFERENCE_PO_NO, d.FOREMAN, 
a.SCHD_CAPACITY_ID, a.SCHD_DAYS_NUMBER, a.SCHD_QTY, b.ORDER_NO, b.CUSTOMER_NAME, b.CUSTOMER_PROGRAM, b.PRODUCT_ITEM_CODE, 
...
LEFT JOIN remark_detail o ON o.SCHD_ID = a.SCHD_ID AND o.SHIFT = :shift
LEFT JOIN (
SELECT N.ENDLINE_SCHD_ID, 
CASE WHEN N.ENDLINE_PORD_TYPE = 'N'  THEN SUM(N.ENDLINE_OUT_QTY) END AS NORMAL_OUTPUT,
CASE WHEN N.ENDLINE_PORD_TYPE = 'O'  THEN SUM(N.ENDLINE_OUT_QTY)  END AS OT_OUTPUT
FROM(
	SELECT  
	a.ENDLINE_SCHD_ID, a.ENDLINE_OUT_TYPE, a.ENDLINE_PORD_TYPE, a.ENDLINE_PLAN_SIZE, a.ENDLINE_OUT_QTY
	FROM qc_endline_output a 
	WHERE a.ENDLINE_OUT_TYPE = 'RTT' AND a.ENDLINE_OUT_UNDO IS NULL 
	AND a.ENDLINE_ID_SITELINE = :idstieline AND a.ENDLINE_SCHD_DATE = :plannDate AND a.ENDLINE_PORD_TYPE = 'N'
	UNION ALL
	SELECT 
	 a.ENDLINE_SCHD_ID, a.ENDLINE_OUT_TYPE, a.ENDLINE_PORD_TYPE, a.ENDLINE_PLAN_SIZE, a.ENDLINE_OUT_QTY
	FROM qc_endline_output a 
	WHERE a.ENDLINE_OUT_TYPE <> 'BS' AND a.ENDLINE_OUT_UNDO IS NULL AND a.ENDLINE_REPAIR = 'Y'
	AND a.ENDLINE_ID_SITELINE = :idstieline AND a.ENDLINE_SCHD_DATE = :plannDate AND a.ENDLINE_PORD_TYPE = 'N'
	UNION ALL
	SELECT
	a.ENDLINE_SCHD_ID, a.ENDLINE_OUT_TYPE, a.ENDLINE_PORD_TYPE, a.ENDLINE_PLAN_SIZE, a.ENDLINE_OUT_QTY
	FROM qc_endline_output a 
	WHERE a.ENDLINE_OUT_TYPE = 'RTT' AND a.ENDLINE_OUT_UNDO IS NULL 
	AND a.ENDLINE_ID_SITELINE = :idstieline AND a.ENDLINE_SCHD_DATE = :plannDate AND a.ENDLINE_PORD_TYPE = 'O'
	UNION ALL
	SELECT 
	a.ENDLINE_SCHD_ID, a.ENDLINE_OUT_TYPE, a.ENDLINE_PORD_TYPE, a.ENDLINE_PLAN_SIZE, a.ENDLINE_OUT_QTY
	FROM qc_endline_output a 
...
) p ON a.SCHD_ID = p.ENDLINE_SCHD_ID
WHERE a.SCHD_PROD_DATE = :plannDate   AND e.MP_DATE = :plannDate  AND d.SITE_NAME = :sitename AND d.LINE_NAME = :linename
)n `;
// export const QueryQcEndlineDaily = `
// SELECT *,ROUND(n.PLAN_MP*n.PLAN_WH*60/n.PLAN_SEW_SMV) PLAN_TARGET,
// 		ROUND(n.PLAN_MP_OT*n.PLAN_WH_OT*60/n.PLAN_SEW_SMV) PLAN_TARGET_OT,
// 		ROUND(n.ACT_MP*n.PLAN_WH*60/n.PLAN_SEW_SMV) ACT_TARGET,
// 		ROUND(n.ACT_MP_OT*n.PLAN_WH_OT*60/n.PLAN_SEW_SMV) ACT_TARGET_OT
//  FROM (
...
// 	CASE WHEN ISNULL(m.PLAN_MP) THEN e.PLAN_MP ELSE m.PLAN_MP END PLAN_MP,
// 	CASE WHEN ISNULL(f.PLAN_WH) THEN e.PLAN_WH ELSE f.PLAN_WH END PLAN_WH,
// 	m.PLAN_MP_OT, f.PLAN_WH_OT,  m.ACT_MP, m.ACT_MP_OT, o.PLAN_REMARK
// 	FROM weekly_prod_sch_detail a
// 	LEFT JOIN viewcapacity b ON a.SCHD_CAPACITY_ID = b.ID_CAPACITY
// 	LEFT JOIN item_smv_header c ON c.ORDER_NO = b.ORDER_NO
// 	LEFT JOIN item_siteline d ON a.SCHD_ID_SITELINE = d.ID_SITELINE
