...
export const QueryPackInDaily = `SELECT  a.SCH_ID, a.SCHD_ID, b.BUYER_CODE, g.SCHD_SITE, e.LINE_NAME, 
f.ORDER_REFERENCE_PO_NO ORDER_REF, f.ITEM_COLOR_NAME ORDER_COLOR, b.ORDER_STYLE,
sum(b.ORDER_QTY) ORDER_QTY, DATE(a.PACKING_SCAN_TIME) SCAN_DATE
FROM scan_packing_in a
LEFT JOIN order_detail b ON a.BARCODE_SERIAL = b.BARCODE_SERIAL 
LEFT JOIN weekly_prod_sch_detail g ON a.SCHD_ID = g.SCHD_ID
LEFT JOIN viewcapacity f ON f.ID_CAPACITY = g.SCHD_CAPACITY_ID
LEFT JOIN xref_user_web d ON a.PACKING_SCAN_BY = d.USER_ID 
LEFT JOIN item_siteline e ON e.ID_SITELINE = g.SCHD_ID_SITELINE
WHERE DATE(a.PACKING_SCAN_TIME) BETWEEN :startdate AND :enddate AND 
e.SITE_NAME = :sitename
GROUP BY a.SCH_ID, a.SCHD_ID`;
