...
LEFT JOIN (
  SELECT a.SCHD_ID, b.ORDER_SIZE, COUNT(*) RETURN_COUNT
  FROM scan_sewing_return a 
  LEFT JOIN order_detail b ON b.BARCODE_SERIAL = a.BARCODE_SERIAL
  WHERE a.SCHD_ID IN (
    SELECT a.SCHD_ID FROM weekly_prod_sch_detail a 
  ...
  GROUP BY a.SCHD_ID
) m ON m.SCHD_ID = a.SCHD_ID AND m.ORDER_SIZE = b.ORDER_SIZE
WHERE g.SCHD_PROD_DATE = :schDate AND a.SEWING_SCAN_LOCATION = :sitename AND 
e.LINE_NAME LIKE :linename
...
LEFT JOIN (
  ...
  GROUP BY a.SCHD_ID
) m ON m.SCHD_ID = a.SCHD_ID AND m.ORDER_SIZE = a.ORDER_SIZE
...
export const QueryGetQrPendding = `SELECT a.BARCODE_SERIAL, h.BUNDLE_SEQUENCE, a.SCH_ID, a.SCHD_ID, b.BUYER_CODE, b.SITE_LINE, g.SITE_NAME, e.LINE_NAME,
