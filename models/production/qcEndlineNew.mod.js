...
FROM (
	...
	IFNULL(d.BS, 0) AS BS,
	CASE WHEN e.BARCODE_SERIAL THEN IFNULL(e.TRANSFER_QTY, b.ORDER_QTY) ELSE 0 END AS TRANSFER_QTY,
...
FROM (
	...
	FROM (
		SELECT a.SCH_ID, a.SCHD_ID,DATE(a.SEWING_SCAN_TIME) SCAN_DATE, d.BUYER_CODE, d.ORDER_COLOR,
		 a.BARCODE_SERIAL, d.ORDER_SIZE, d.MO_NO, d.ORDER_QTY,	
	...
	LEFT JOIN (
					...
					WHERE a.BARCODE_SERIAL IN (
						  SELECT 
							n.BARCODE_SERIAL
							FROM (
								SELECT  a.BARCODE_SERIAL, d.MO_NO, d.ORDER_QTY,	CASE WHEN F.BARCODE_SERIAL THEN IFNULL(F.TRANSFER_QTY, d.ORDER_QTY) ELSE 0 END AS TRANSFER_QTY
								FROM scan_sewing_in a 
								LEFT JOIN weekly_prod_schedule b ON a.SCH_ID = b.SCH_ID
								LEFT JOIN item_siteline c ON b.SCH_ID_SITELINE = c.ID_SITELINE
								LEFT JOIN view_order_detail d ON d.BARCODE_SERIAL = a.BARCODE_SERIAL
