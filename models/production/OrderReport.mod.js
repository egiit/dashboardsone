...
FROM ( 
	...
	GROUP BY  a.PRODUCTION_MONTH, a.MANUFACTURING_SITE, a.ORDER_NO, a.ORDER_REFERENCE_PO_NO
	ORDER BY a.PRODUCTION_MONTH, a.MANUFACTURING_SITE, a.CUSTOMER_NAME, a.CUSTOMER_DIVISION, a.ORDER_NO, a.ORDER_REFERENCE_PO_NO
) FA 
LEFT JOIN (
	...
	SUM(B.ORDER_QTY) AS QR_QTY,
	SUM(C.ORDER_QTY) AS CUTTING_OUTPUT_QTY,
	SUM(D.ORDER_QTY) AS SEWING_IN,
	SUM(E.ORDER_QTY) AS SEWING_OUT,
	SUM(F.ORDER_QTY) AS PACKING_IN
	FROM (
	...
	LEFT JOIN (
        SELECT a.MO_NO, a.ORDER_SIZE, SUM(a.ORDER_QTY) ORDER_QTY
        FROM view_order_detail a 
        GROUP BY a.MO_NO
    ) B ON A.MO_NO = B.MO_NO
    LEFT JOIN (
        SELECT b.MO_NO, b.ORDER_SIZE, SUM(b.ORDER_QTY) ORDER_QTY
    ...
    LEFT JOIN (
        ...
        LEFT JOIN view_order_detail b ON a.BARCODE_SERIAL = b.BARCODE_SERIAL
        GROUP BY b.MO_NO
    ) E ON A.MO_NO = E.MO_NO
    LEFT JOIN (
        SELECT b.MO_NO, b.ORDER_SIZE, SUM(b.ORDER_QTY) ORDER_QTY
        FROM scan_sewing_out a 
        LEFT JOIN view_order_detail b ON a.BARCODE_SERIAL = b.BARCODE_SERIAL
        GROUP BY b.MO_NO
    ) F ON A.MO_NO = F.MO_NO
    GROUP BY A.PRODUCTION_MONTH, A.MANUFACTURING_SITE, A.CUSTOMER_NAME,
    A.ORDER_NO, A.ORDER_REFERENCE_PO_NO
...
FROM ( 
    ...
    SUM(n.SCH_QTY) SCH_QTY
    FROM viewcapacity a 
    LEFT JOIN (
        SELECT a.ID_CAPACITY, a.SIZE_CODE,   SUM(a.SCH_SIZE_QTY) SCH_QTY
        FROM weekly_sch_size a  
        WHERE a.ID_CAPACITY IN (
                SELECT ID_CAPACITY FROM viewcapacity WHERE ORDER_REFERENCE_PO_NO = :orderRefPoNo 
            )
        GROUP BY a.ID_CAPACITY, a.SIZE_CODE
        ) n ON a.ID_CAPACITY = n.ID_CAPACITY
    WHERE a.PRODUCTION_MONTH = :prodMonth AND
        a.MANUFACTURING_SITE = :capSite AND
        a.ORDER_REFERENCE_PO_NO = :orderRefPoNo 
    GROUP BY  a.PRODUCTION_MONTH, a.MANUFACTURING_SITE, a.ORDER_NO, a.ORDER_REFERENCE_PO_NO, n.SIZE_CODE
    ORDER BY a.PRODUCTION_MONTH, a.MANUFACTURING_SITE, a.CUSTOMER_NAME, a.CUSTOMER_DIVISION, a.ORDER_NO, a.ORDER_REFERENCE_PO_NO, n.SIZE_CODE
) FA 
LEFT JOIN (
    -- DETAIL PER SIZE
SELECT A.PRODUCTION_MONTH, A.MANUFACTURING_SITE, A.CUSTOMER_NAME,
...
FROM (
    SELECT a.PRODUCTION_MONTH, a.MANUFACTURING_SITE, a.CUSTOMER_NAME, a.ORDER_NO, a.ORDER_REFERENCE_PO_NO, 
    a.MO_NO, a.SIZE_CODE,  a.PLAN_EXFACTORY_DATE, a.ITEM_COLOR_CODE, SUM(a.ORDER_QTY) ORDER_QTY, SUM(a.MO_QTY) MO_QTY
    FROM order_po_listing_size a 
    WHERE a.PRODUCTION_MONTH = :prodMonth AND
        a.MANUFACTURING_SITE = :capSite AND
        a.ORDER_REFERENCE_PO_NO = :orderRefPoNo 
    GROUP BY a.MO_NO, a.SIZE_CODE
    ) A 
    LEFT JOIN (
        SELECT a.MO_NO, a.ORDER_SIZE, SUM(a.ORDER_QTY) ORDER_QTY
        FROM view_order_detail a 
        GROUP BY a.MO_NO, a.ORDER_SIZE
    ) B ON A.MO_NO = B.MO_NO AND A.SIZE_CODE = B.ORDER_SIZE
    LEFT JOIN (
        SELECT b.MO_NO, b.ORDER_SIZE, SUM(b.ORDER_QTY) ORDER_QTY
        FROM order_qr_generate a 
        LEFT JOIN view_order_detail b ON a.BARCODE_SERIAL = b.BARCODE_SERIAL
        GROUP BY b.MO_NO, b.ORDER_SIZE
    ) C ON A.MO_NO = C.MO_NO AND A.SIZE_CODE = C.ORDER_SIZE
    LEFT JOIN (
        SELECT b.MO_NO, b.ORDER_SIZE, SUM(b.ORDER_QTY) ORDER_QTY
        FROM scan_sewing_in a 
        LEFT JOIN view_order_detail b ON a.BARCODE_SERIAL = b.BARCODE_SERIAL
        GROUP BY b.MO_NO, b.ORDER_SIZE
    ) D ON A.MO_NO = D.MO_NO AND A.SIZE_CODE = D.ORDER_SIZE
    LEFT JOIN (
        SELECT b.MO_NO, b.ORDER_SIZE, SUM(b.ORDER_QTY) ORDER_QTY
        FROM scan_sewing_out a 
        LEFT JOIN view_order_detail b ON a.BARCODE_SERIAL = b.BARCODE_SERIAL
        GROUP BY b.MO_NO, b.ORDER_SIZE
    ) E ON A.MO_NO = E.MO_NO AND A.SIZE_CODE = E.ORDER_SIZE
    LEFT JOIN (
        SELECT b.MO_NO, b.ORDER_SIZE, SUM(b.ORDER_QTY) ORDER_QTY
        FROM scan_sewing_out a 
        LEFT JOIN view_order_detail b ON a.BARCODE_SERIAL = b.BARCODE_SERIAL
        GROUP BY b.MO_NO, b.ORDER_SIZE
    ) F ON A.MO_NO = F.MO_NO AND A.SIZE_CODE = F.ORDER_SIZE
    GROUP BY A.PRODUCTION_MONTH, A.MANUFACTURING_SITE, A.CUSTOMER_NAME,
    A.ORDER_NO, A.ORDER_REFERENCE_PO_NO, A.SIZE_CODE
...
export const QryGetOrderByCapacity = `
SELECT 
